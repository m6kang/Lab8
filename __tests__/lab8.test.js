describe('Basic user flow for SPA ', () => {
  ...
  it('Test6: On first Entry page - checking <body> element classes', async () => {
    ...
    const bclass = await page.$eval('body', el => el.className);
    expect(bclass).toBe('single-entry');
  });
  ...
  it('Test17: Clicking the back button, then forward button, new URL should be /#entry2', async() => {
    await page.goBack();
  ...
  it('Test18: Clicking the header - checking page header title', async ()=> {
    await page.click('h1');
    const title = await page.$eval('body > header > h1', el => el.textContent);
    expect(title).toBe('Journal Entries');
  },10000);
